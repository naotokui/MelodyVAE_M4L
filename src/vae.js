...
async function loadAndTrain(train_data_onset, train_data_velocity) {
...
class ConditionalVAE {
  ...
  build(modelConfig) {
    ...
    const x1Merged = concatLayer.apply([x1On, x1Vel]);
    ...
    const encoderInputs = [encoderInputsOn, encoderInputsVel];
    ...
    const decoderOutputs = [decoderOutputsOn, decoderOutputsVel];
  ...
  vaeLoss(yTrue, yPred) {
    return tf.tidy(() => {
      ...
      const total_loss = tf.mean(onset_loss.add(velocity_loss).add(kl_loss)); // averaged in the batch
      return total_loss;
    });
  }
  ...
  //   const updateProgressBar = config.updateProgressBar;
    ...
    for (let i = 0; i < epochs; i++) {
      ...
      let batchInputOn, batchInputVel;
      let trainLoss;
      let epochLoss;
      ...
      for (let j = 0; j < numBatch; j++) {
        ...
        trainLoss = await optimizer.minimize(() => this.vaeLoss([batchInputOn, batchInputVel], 
            this.apply([batchInputOn, batchInputVel])), true);
